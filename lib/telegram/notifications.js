...
import getBot, { telegramApi } from './client';
import { createClient } from '@/lib/supabase/server';
import { wantsChannel } from '@/lib/notifications/preferences';

/**
 * Envía un mensaje de Telegram usando el método apropiado según el entorno
...
async function sendTelegramMessage(chatId, text, options = {}) {
  ...
  if (isProduction) {
    // En producción, usar telegramApi (fetch) para evitar problemas con serverless
    return await telegramApi.sendMessage(chatId, text, options);
  } else {
    // En desarrollo, usar getBot() con polling
    const bot = getBot();
    if (!bot) {
      throw new Error('Bot de Telegram no disponible en desarrollo');
    }
...
export async function enviarNotificacionTelegramNoticia(
  ...
  if (!process.env.TELEGRAM_BOT_TOKEN) {
    console.log('⚠️ TELEGRAM_BOT_TOKEN no configurado. Notificación omitida.');
    return { success: false, sent: 0, errors: 0, message: 'Token no configurado' };
  }
