...
export async function enviarNotificacionTelegramNoticia(
  ...
  try {
    ...
    const { data: usuarios, error } = await supabase
      .from('usuarios')
      .select('telegram_chat_id, nombres, apellidos')
      .eq('rol', 'vecino')
      .eq('estado', 'activo')
      .not('telegram_chat_id', 'is', null)
      .in('preferencia_notificacion', ['telegram', 'ambos']);
    ...
    console.log(`📱 Enviando notificación Telegram a ${usuarios.length} usuarios`);
...
export async function enviarNotificacionTelegramAviso(
  ...
  try {
    ...
    console.log(`📱 Enviando aviso Telegram a ${usuarios.length} usuarios`);
